        ...
        with open(file_path, 'r') as file:
            ...
            for line in file:
                ...
                if line.startswith("Nome:"):
                    ...
                    continue
                elif line.startswith("Referto:"):
                    paziente_info['Referto'] = line.split(":")[1].strip()
                    continue
...
def generate_pdf(image_path, paziente_info):
    ...
    # Title
    title_style = ParagraphStyle(name='Title', fontName='Times-Bold', fontSize=16, leading=20, spaceAfter=6, alignment=1)
    elements.append(Paragraph("Referto Audiometrico", title_style))
    elements.append(HRFlowable(width="100%", thickness=1, color=colors.black))
    elements.append(Spacer(1, 12))

    # Patient information
    bold_style = ParagraphStyle(name='Bold', fontName='Times-Bold', fontSize=10)
    info_table_data = [
        [Paragraph("Paziente:", bold_style), paziente_info['Nome'] + " " + paziente_info['Cognome']],
        [Paragraph("CF:", bold_style), paziente_info['CF']],
        [Paragraph("Data Esame:", bold_style), paziente_info['Data_Esame']],
        [Paragraph("Tel:", bold_style), paziente_info['Telefono']],
        [Paragraph("Tessera Sanitaria:", bold_style), paziente_info['Tessera_Sanitaria']]
    ]
    info_table = Table(info_table_data, colWidths=[190, 310], hAlign='CENTER')
    ...
    # Audiogram image
    elements.append(Image(image_path, width=450, height=250))
    elements.append(Spacer(1, 12))

    # Legend
    bold_small_style = ParagraphStyle(name='BoldSmall', fontName='Times-Roman', fontSize=10)
    legend_text = """
    <b>Legenda:</b><br/>
    ...
    <b>Non rilev. (▼ nero):</b> Soglia non rilevabile
    """
    elements.append(Paragraph(legend_text, bold_small_style))
    elements.append(Spacer(1, 12))

    # Report note
    report_note = f"""
    <b>REFERTO:</b> {paziente_info['Referto']}
    """
    elements.append(Paragraph(report_note, bold_small_style))
    elements.append(Spacer(1, 12))
    elements.append(Paragraph("<b>NOTE:</b>", bold_small_style))
    elements.append(Spacer(1, 24))

    # Footer
    firma_style = ParagraphStyle(name='Firma', fontName='Times-Roman', fontSize=10, alignment=2)
    elements.append(Paragraph("Firma del medico: _______________________", firma_style))
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(f"{datetime.now().strftime('%d/%m/%Y %H:%M')}", firma_style))
    elements.append(Spacer(1, 12))
    elements.append(HRFlowable(width="100%", thickness=1, color=colors.black))
...
def send_email():
    sender = "pasquale.valentino@otofarma.it"
    receiver = "pasquale.valentino@otofarma.it"
    subject = "Referto Audiometrico"
    body = "In allegato il referto audiometrico."
    ...
    try:
        smtp_server = "smtp.otofarma.it"
        smtp_port = 587
        smtp_username = "pasquale.valentino@otofarma.it"
        smtp_password = "otofarmaspa1025"
        ...
        server.quit()
        return "Email inviata con successo", 200
    except Exception as e:
        return f"Errore nell'invio dell'email: {str(e)}", 500
